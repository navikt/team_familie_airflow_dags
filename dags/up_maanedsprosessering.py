...
# Finner hvilken periode_type passer med datoen DAGen kjører
def detect_period_type(now: pendulum.DateTime) -> str:
    day = now.day
    month = now.month
    ...
    # Kvartalskjøring (1. april)
    if day == 1 and month == 4:
        return "K"

    # Halvårskjøring (1. juli)
...
# Beregner hva variablene er basert på periode_type 
def compute_period_bounds(periode_type: str, now_oslo: pendulum.DateTime, forskyvningsdager: int) -> tuple[str, str, str, str]:
    max_vedtaksdato = now_oslo.add(days=forskyvningsdager).format("YYYYMMDD") # Gjelder alle
    periode_tom = now_oslo.subtract(months=1).format("YYYYMM") # Gjelder alle

    #PERIODE_TYPE	STAT_PERIODE	Forskyvningsdager	MAX_VEDTAKSDATO	FRA	    TIL
    #K	            K20261	        1	                20260401	    202601	202603
    #H	            H20261	        1	                20260701	    202601	202606
    #S	            S20269	        1	                20261001	    202601	202609
    #A	            A2026	        1	                20270101	    202601	202612

    # År 
    if periode_type == "A":
        periode_fom = now_oslo.subtract(months=12).format("YYYYMM") # E.g. 20270101 = 202601
...
# Automatisk beregning av riktig periode å kjøre hvilke parametere
periode_type = detect_period_type(now_oslo)
if periode_type is None:
    raise AirflowSkipException(f"Ingen periodetype treffer i dag, kun 1. jan., apr., jul. eller okt. Skipper hele DAGen. :tada:")

periode_fom, periode_tom, max_vedtaksdato, periode_type = compute_period_bounds(
    periode_type, now_oslo, forskyvningsdager
)

up_variabler_mnd = Variable.get("up_variabler_mnd", deserialize_json=True)
DEFAULT_FORSKYVNING = 1 # Hardkodede defaults, kan forbedre dette senere
DEFAULT_GYDLIG_FLAGG = 1
forskyvningsdager = up_variabler_mnd.get("forskyvningsdager") or DEFAULT_FORSKYVNING
gyldig_flagg = up_variabler_mnd.get("gyldig_flagg") or DEFAULT_GYDLIG_FLAGG
...
with DAG(
    ...
    def notification_end():
        slack_info(
            message=(f"Prosessering av Ungdomsprogrammet er ferdig! :tada:")
        )
