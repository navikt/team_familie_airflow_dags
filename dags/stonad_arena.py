...
periode = fam_ef_stonad_arena_methods.get_periode() #"""'{{"periode":{}}}'""".format(fam_ef_stonad_arena_methods.get_periode())
...
with DAG(
    ...
    dbt_run = create_knada_python_pod_operator(
        ...
        do_xcom_push = True, 
        extra_envs={
            'DBT_COMMAND': "run --vars \"\"\"{'periode':" + '{}{}\"\"\"'.format(periode,'}'), #"""run --vars "{'periode':""" + '{}"'.format(periode),  #"""run --vars '{{"periode":{}}}'""".format(periode), #"""run --vars '{{"periode":"{}}}'""".format(periode), #'run --vars {}'.format(periode), # 'samme som i dbt terminalen men uten dbt. ex) dbt run -model blabla'
            'LOG_LEVEL': 'DEBUG',
